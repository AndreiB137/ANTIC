...
def run_kdv(cfg: ExperimentConfig, run_checkpoint: bool = False):
    """Run the full KdV ANTIC compression experiment."""
    ...
    print(f"[solver] kdv  N={solver.N}  dt={solver.dt}")
    ...
    if run_checkpoint:
        ...
        positions = jnp.array([2.0 * jnp.pi, 6.0 * jnp.pi,
                            12.0 * jnp.pi, 18.0 * jnp.pi])
        state = solver.init_condition(velocities, positions)

    # ---- selector -----------------------------------------------
    selector = build_selector(cfg.selector)
    initial_field = solver.extract(state)
    if cfg.selector.type != "none":
        if run_checkpoint:
            selector.load_state(cfg.training.save_dir + "/checkpoint/selector")
        else:
            selector.init(initial_field, solver)
    ...
    if cfg.training.filter == 'lora':
        add_lora_to_model(model, rank=cfg.training.rank, rngs=nnx.Rngs(cfg.seed + 10))
    ...
    while True:
        ...
        # ---- NaN debugging: reset LoRA if NaN detected ----
        if jnp.isnan(loss):
